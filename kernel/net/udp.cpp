	...
	if ( af == AF_INET )
	{
		     ...
		     new_local.in.sin_addr.s_addr != htobe32(INADDR_BROADCAST) )
		{
			...
			for ( unsigned int i = 1; i < netifs_count; i++ )
			{
				...
				struct in_addr if_brodcast_ip;
				if_brodcast_ip.s_addr = netif->cfg.inet.address.s_addr |
				                        ~netif->cfg.inet.subnet.s_addr;
				if ( memcmp(&netif->cfg.inet.address, &new_local.in.sin_addr,
				            sizeof(struct in_addr)) == 0 ||
				     memcmp(&if_brodcast_ip, &new_local.in.sin_addr,
				            sizeof(struct in_addr)) == 0 )
				{
					found = true;
					break;
                                    ...
                                    int flags)
{
	...
	if ( af == AF_INET )
	{
		if ( be16toh(sendto.in.sin_port) == 0 )
			return errno = EADDRNOTAVAIL, -1;
		if ( !IPv4::GetSourceIP(&local.in.sin_addr, &sendto.in.sin_addr,
	...
	if ( af == AF_INET )
	{
		...
		checksum = IPv4::ipsum_buf(checksum, &sendto.in.sin_addr,
		                           sizeof(struct in_addr));
	}
	else if ( af == AF_INET6 )
	{
		...
		checksum = IPv4::ipsum_buf(checksum, &sendto.in6.sin6_addr,
		                           sizeof(struct in6_addr));
	}
	else
		return errno = EAFNOSUPPORT, -1;
	checksum = IPv4::ipsum_word(checksum, IPPROTO_UDP);
	checksum = IPv4::ipsum_word(checksum, pkt->length);
	checksum = IPv4::ipsum_buf(checksum, out, pkt->length);
	checksum = IPv4::ipsum_finish(checksum);
	if ( checksum == 0x0000 )
		checksum = 0xFFFF;
	...
	if ( af == AF_INET )
	{
		if ( !IPv4::Send(pkt, &sendfrom.in.sin_addr, &sendto.in.sin_addr,
                ...
                bool dst_broadcast)
{
	...
	if ( hdr.uh_sum )
	{
		...
		sum = IPv4::ipsum_word(sum, IPPROTO_UDP);
		sum = IPv4::ipsum_word(sum, hdr.uh_ulen);
	...
	if ( inlen < hdr.uh_ulen )
		return;
	// TODO: Truncate the packet to hdr.uh_ulen!
	...
	from_addr.sin_port = htobe16(hdr.uh_dport);
