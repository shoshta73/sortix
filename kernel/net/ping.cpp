                                     ...
                                     int flags)
{
	...
	hdr.checksum = htobe16(IPv4::ipsum(out, pkt->length));
	memcpy(out, &hdr, sizeof(hdr));
	(void) flags;
	if ( af == AF_INET )
	{
                ...
                bool dst_broadcast)
{
	...
	if ( hdr.type == ICMP_TYPE_ECHO_REQUEST )
	{
		...
		if ( !IPv4::GetSourceIP(dst, src, &sendfrom, 0) )
			return;
		...
		reply.checksum = htobe16(IPv4::ipsum(out, out_pkg->length));
		memcpy(out, &reply, sizeof(reply));
		if ( !IPv4::Send(out_pkg, &sendfrom, src, IPPROTO_ICMP, 0, false) )
			return;
		return;
	}
