...
//
//       - TCP_OFFSET_DECODE.
//       - Finish socket closing and lifetime.
//       - Implement waiting 2 MSL after socket is closed.
//       - Implement sending back RST and such.
//       - Improve retransmission logic under error conditions.
...
// TODO: Document tcp_lock, and lifetimes.
...
// TODO: Document the socket lifetime.
// TODO: Document the state in tcp(4).
class TCPSocket
{
...
public:
	...
	void TransmitWork();
	void OnTimer();
	// TODO: Review this function carefully.
	inline bool needs_destruction()
	{
...
private:
	...
	// TODO: Confirm the above comment is true.
	bool transmit_do_schedule_worker;

	// Whether a transmission is currently happening.
	// TODO: Confirm the above comment is true.
	bool transmit_ongoing;

	// Whether the socket should be deleted when it's closed.
...
TCPSocket::~TCPSocket()
{
	// TODO: TCP_STATE_LISTEN can't reach this location, as all
	//       needs_destruction() requires the CLOSED state, and close_unlocked
	//       puts us in the CLOSED state. Let's make it official that closing
	//       a listening socket puts it in the CLOSED state?
...
// TODO: Review this function carefully.
bool TCPSocket::close_unlocked() // tcp_lock taken
{
	if ( state != TCP_STATE_CLOSED &&
	     state != TCP_STATE_LISTEN &&
	     state != TCP_STATE_SYN_SENT &&
	     state != TCP_STATE_SYN_RECV /* TODO: ??? */ )
	{
	...
	if ( timer_armed && timer.TryCancel() )
		timer_armed = false;
	if ( transmit_ongoing || timer_armed )
	{
		delete_on_close = true;
		return false;
	}
	// TODO: Is this right in the TCP_STATE_SYN_SENT and TCP_STATE_SYN_RECV
	//       cases? Should the socket be kept around for some MSLs or such?
	state = TCP_STATE_CLOSED;
	destroy();
	return true;
}
...
void TCPSocket::destroy() // tcp_lock taken
{
	...
	while ( connecting_half || connecting_ready )
	{
		...
		// TODO: This should send RST to the other side. (But probably not for
		//       half-open connections).
		// TODO: According to what should that happen?
		socket->close_unlocked();
		backlog_used--;
	}
...
bool TCPSocket::Transmit()
{
	        ...
	        recv_wnd != recv_wndlast )
	{
		...
		if ( af == AF_INET )
		{
			...
			{
				// TODO: Retry after timer. Close if permanent failure?
				return false;
			}
		}
		// TODO: IPv6 support.
		...
		if ( af == AF_INET )
		{
			...
			{
				// TODO: Retry after timer. Close if permanent failure?
				// TODO: Should the initial SYN fail quick & hard if permanent?
				sockerr = errno;
		...
		// TODO: Also set timer to retry in case of any errors above.
		if ( mod32_lt(send_una, send_pos) && !timer_armed )
			SetTimer();
	}
	return true;
}
...
void TCPSocket::TransmitWork()
{
	...
	transmit_ongoing = false;
	Transmit();
	// TODO: What if the timer needs to be cancelled due to the new close state?
...
void TCPSocket::OnTimer()
{
	...
	if ( mod32_lt(send_una, send_pos) )
	{
		...
		if ( 6 /* Or some suitable constant */ < retransmissions )
		{
			// TODO: Consider the connection closed.
			sockerr = ETIMEDOUT;
			kthread_cond_broadcast(&transmit_cond);
			kthread_cond_broadcast(&receive_cond);
			poll_channel.Signal(PollEventStatus());
		...
		send_pos = send_una;
		Transmit();
		// TODO: needs_destruction() could be true here?
	}
}
                              ...
                              union tcp_sockaddr* pkt_dst) // tcp_lock locked
{
	...
	else if ( state == TCP_STATE_SYN_SENT )
	{
		...
		if ( hdr.th_flags & TH_ACK )
		{
			...
			retransmissions = 0;
			SetTimer();
			// TODO: Dequeue any segments on the retransmission queue before
			//       send_una.
			// TODO: How can the above happen?
                        ...
                        int flags)
{
	if ( flags & ~(MSG_PEEK | MSG_WAITALL) ) // TODO: MSG_OOB.
		return errno = EINVAL, -1;
	kthread_mutex_lock(&tcp_lock);
	size_t result = recv_unlocked(ctx, buf, count, flags);
	bool do_schedule_worker = transmit_do_schedule_worker;
	transmit_do_schedule_worker = false;
	kthread_mutex_unlock(&tcp_lock);
                        ...
                        int flags)
{
	...
	if ( flags & ~(MSG_NOSIGNAL) ) // TODO: MSG_OOB, MSG_DONTROUTE.
		return errno = EINVAL, -1;
	kthread_mutex_lock(&tcp_lock);
	size_t result = send_unlocked(ctx, buf, count, flags);
	bool do_schedule_worker = transmit_do_schedule_worker;
	transmit_do_schedule_worker = false;
	kthread_mutex_unlock(&tcp_lock);
	if ( do_schedule_worker )
...
short TCPSocket::PollEventStatus()
{
	...
	if ( incoming_used )
		status |= POLLIN | POLLRDNORM;
	if ( outgoing_used < sizeof(outgoing) )
                ...
                bool dst_broadcast)
{
	...
	// Destroy the socket if needed or schedule a transmit if needed.
	if ( socket->needs_destruction() )
	{
		socket->destroy();
		delete socket;
	}
...
Ref<Inode> Socket(int af)
{
	...
	if ( !result )
	{
		// destroy() is not called because there is no state that needs to be
		// cleaned up that requires the tcp_lock to be taken.
