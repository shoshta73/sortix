                               ...
                               bool* out_broadcast)
{
	...
	in_addr_t if_brodcast_ip =
		netif->cfg.inet.address.s_addr | ~netif->cfg.inet.subnet.s_addr;
	if ( !memcmp(&if_brodcast_ip, src, sizeof(in_addr_t)) )
		return false;
	if ( netif->ifinfo.type != IF_TYPE_LOOPBACK )
	{
	...
	*out_broadcast = false;
	...
	if ( memcmp(&netif->cfg.inet.address, &any_ip, sizeof(in_addr_t)) != 0 )
	{
		...
		if ( !memcmp(&if_brodcast_ip, dst, sizeof(in_addr_t)) )
			return *out_broadcast = true, true;
	}
	return false;
}
            ...
            bool dst_broadcast)
{
	...
	if ( pkt->length - pkt->offset < sizeof(hdr) )
		return;
	...
	if ( IPV4_VERSION(hdr.version_ihl) != 4 )
		return;
	// Verify the header length isn't smaller than the header.
	uint8_t ihl = IPV4_IHL(hdr.version_ihl);
	if ( ihl < sizeof(hdr) / 4 )
		return;
	// Verify the packet isn't smaller than the header length.
	if ( pkt->length - pkt->offset < ihl * 4 )
		return;
	// Verify the packet length isn't smaller than the datagram.
	if ( pkt->length - pkt->offset < hdr.length )
		return;
          ...
          bool broadcast)
{
	...
	if ( netif->ifinfo.type == IF_TYPE_LOOPBACK )
	{
		...
		memset(&localaddr, 0, sizeof(broadcast));
