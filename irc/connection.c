                  ...
                  size_t message_size)
{
	...
	{
		ssize_t amount = 0;
		if ( irc_connection->fd == irc_connection->fd )
			amount = send(fd, message, message_size, MSG_NOSIGNAL);
		else
			amount = write(fd, message, message_size);
...
void irc_command_botprivmsgf(struct irc_connection* irc_connection,
                             const char* where,
                             const char* what_format,
                             ...)

{
	...
	va_start(ap, what_format);
	char msg1[512];
	vsnprintf(msg1, sizeof(msg1), what_format, ap);
	if ( msg1[0] != 0x01 )
	{
		char msg2[512];
		snprintf(msg2, sizeof(msg2), "\u200B""%s", msg1);
		irc_command_privmsg(irc_connection, where, msg2);
	}
	else
	{
		irc_command_privmsg(irc_connection, where, msg1);
	}
	va_end(ap);
}
                      ...
                      const char* message)
{
	...
	if ( irc_connection->fd == irc_connection->fd ) // Socket, not pipe
