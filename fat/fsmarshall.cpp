...
void HandleReadDir(int chl, struct fsm_req_readdirents* msg, Filesystem* fs)
{
	...
	union
	{
		...
		uint8_t padding[sizeof(struct dirent) + FAT_UTF8_NAME_MAX + 1];
	};
	memset(&kernel_entry, 0, sizeof(kernel_entry));

	struct position position;
	position.cluster = inode->first_cluster;
	position.sector = 0;
	position.offset = 0;
	Block* block = NULL;
	...
	while ( inode->ReadDirectory(&block, &position, msg->rec_num ? NULL : name,
