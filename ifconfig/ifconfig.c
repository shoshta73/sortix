...
static void ether_address_print(const struct if_all* all, const void* ptr)
{
	...
	printf("%02x:%02x:%02x:%02x:%02x:%02x",
	       ...
	       addr->ether_addr_octet[1], addr->ether_addr_octet[3],
	       addr->ether_addr_octet[2], addr->ether_addr_octet[5]);
}
...
{
	{ "up", STATUSOFFSET(flags), link_up_print, NULL },
	{ "type", INFOOFFSET(type), link_type_print, NULL },
	{ "id", INFOOFFSET(linkid), link_id_print, NULL },
};

struct configuration ether_configurations[] =
{
	{ "address", CONFIGOFFSET(ether, address),
	  ether_address_print, ether_address_parse },
	// TODO: Real ethernet hardware address.
};
...
struct configuration inet_configurations[] =
{
	{ "address", CONFIGOFFSET(inet, address),
	  inet_address_print, inet_address_parse },
...
int main(int argc, char* argv[])
{
	...
	for ( int d = 0; d < devices_count; d++ )
	{
		...
		if ( ioctl(fd, NIOC_GETINFO, &all.info) < 0 )
			err(1, "%s: ioctl: NIOC_GETLINKINFO", name);
		if ( ioctl(fd, NIOC_GETSTATUS, &all.status) < 0 )
			err(1, "%s: ioctl: NIOC_GETLINKSTAT", name);
