...
void display_composit(struct display* display, struct framebuffer fb)
{
	...
	int pointer_x = display->pointer_x - pointer_hwidth;
	int pointer_y = display->pointer_y - pointer_hheight;
...
void display_keyboard_event(struct display* display, uint32_t codepoint)
{
	...
	#if 1 /* TODO: Useful for development. */
		if ( display->key_lctrl && kbkey == KBKEY_C )
			exit(0);
	#endif
	}
	...
	if ( kbkey && window )
	{
		...
		if ( display->key_lalt )
		{
			struct window* window = display->active_window;
			if ( kbkey == KBKEY_LEFT )
				window_move(window, window->left - 10, window->top);
			if ( kbkey == KBKEY_RIGHT )
				window_move(window, window->left + 10, window->top);
			if ( kbkey == KBKEY_UP )
				window_move(window, window->left, window->top - 10);
			if ( kbkey == KBKEY_DOWN )
				window_move(window, window->left, window->top + 10);
		}
	}
...
{
	if ( display->mouse_byte_count == 0 && !(byte & 1 << 3) )
		return;
	if ( display->mouse_byte_count < 3 )
		display->mouse_bytes[display->mouse_byte_count++] = byte;
	if ( display->mouse_byte_count < 3 )
		return;
	display->mouse_byte_count = 0;
	uint8_t* bytes = display->mouse_bytes;
	int xm = (uint8_t) bytes[1];
	int ym = (uint8_t) bytes[2];
	if ( xm && bytes[0] & (1 << 4) )
		xm = xm - 256;
	if ( ym && bytes[0] & (1 << 5) )
		ym = ym - 256;
	...
	{
		if ( old_pointer_x < window->left )
			continue;
		if ( old_pointer_y < window->top )
			continue;
		if ( old_pointer_x > window->left + (ssize_t) window->width )
			continue;
		if ( old_pointer_y > window->top + (ssize_t) window->height )
			continue;
		break;
	}
	if ( !window )
