...
int main(int argc, char* argv[])
{
	...
	else if ( optind < argc )
	{
		...
		for ( int i = optind; i < argc; i++ )
		{
			...
			if ( !found )
				errx(1, "unknown memory counter: %s", argv[i]);
		}
	}
	else
	{
	...
	// TODO: How about another parameter for the output array? Makes it easier
	//       to keep track of what was requested.
	if ( memusage(counts, num_counters) )
		err(1, "memusage");

	if ( raw && unit == -1 )
		unit = BYTES;

	size_t total = counts[0];
