...
bool RespondTCGetBlobISO(int chl, const char* data, size_t data_size)
{
	data_size = isostrnlen(data, data_size);
	struct fsm_resp_tcgetblob body;
	body.count = data_size;
	return RespondMessage(chl, FSM_RESP_TCGETBLOB, &body, sizeof(body)) &&
	       RespondData(chl, data, data_size);
...
void HandleReadDir(int chl, struct fsm_req_getdents* msg, Filesystem* fs)
{
	...
	if ( (uintmax_t) UINT64_MAX < (uintmax_t) msg->offset )
		msg->offset = UINT64_MAX;
	                             ...
	                             name, &file_type, &inode_id) )
	{
		if ( start_offset <= entry_offset )
		{
			...
			dent->d_ino = inode_id;
			dent->d_type = HostDTFromFsDT(file_type);
...
void HandleTCGetBlob(int chl, struct fsm_req_tcgetblob* msg, Filesystem* fs)
{
	...
	else if ( !strcmp(name, "system-id") )
		RespondTCGetBlobISO(chl, fs->pvd->system_identifier,
		                    sizeof(fs->pvd->system_identifier));
	else if ( !strcmp(name, "volume-id") )
		RespondTCGetBlobISO(chl, fs->pvd->volume_identifier,
		                    sizeof(fs->pvd->volume_identifier));
	else if ( !strcmp(name, "volume-set-id") )
		RespondTCGetBlobISO(chl, fs->pvd->volume_set_identifier,
		                    sizeof(fs->pvd->volume_set_identifier));
	else if ( !strcmp(name, "publisher-id") )
		RespondTCGetBlobISO(chl, fs->pvd->publisher_identifier,
		                    sizeof(fs->pvd->publisher_identifier));
	else if ( !strcmp(name, "data-preparer-id") )
		RespondTCGetBlobISO(chl, fs->pvd->data_preparer_identifier,
		                    sizeof(fs->pvd->data_preparer_identifier));
	else if ( !strcmp(name, "application-id") )
		RespondTCGetBlobISO(chl, fs->pvd->application_identifier,
		                    sizeof(fs->pvd->application_identifier));
	else if ( !strcmp(name, "copyright-id") )
		RespondTCGetBlobISO(chl, fs->pvd->copyright_file_identifier,
		                    sizeof(fs->pvd->copyright_file_identifier));
	else if ( !strcmp(name, "abstract-id") )
		RespondTCGetBlobISO(chl, fs->pvd->abstract_file_identifier,
		                    sizeof(fs->pvd->abstract_file_identifier));
...
void HandlePathConf(int chl, struct fsm_req_pathconf* msg, Filesystem* fs)
{
	...
	case _PC_SYMLINK_MAX: value = 255; break;
	case _PC_TIMESTAMP_RESOLUTION:
		value = 0 <= fs->rr_ext ? 10000000L : 1000000000L; break;
